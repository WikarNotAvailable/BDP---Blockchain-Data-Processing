...
try:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        crypto_df = btc_df.unionByName(eth_df)
        load(crypto_df, 'crypto') # Here we recalcualte both eth and btc df due to 'write'. We could use cache()/persist() to avoid it but it causes memory issues. We can also read eth and btc from parquets
